...
import type { ClaimedVisit } from "@/app/(dashboard)/d/nurse/NurseDashboard";

interface NurseClaimedCardProps {
  visit: ClaimedVisit;
  onClick: () => void;
}

export default function NurseClaimedCard({ visit, onClick }: NurseClaimedCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full rounded-xl border border-gray-200 bg-white p-4 text-left transition-all hover:border-teal-400 hover:shadow-sm"
    >
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-teal-600 font-medium">View &rarr;</span>
      </div>
    </button>
  );
}
