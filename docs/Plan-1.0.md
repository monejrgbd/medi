...
- **To complete:** doctor must enter their actual diagnosis. This is how they close the queue entry. Optionally, the doctor can tag a **follow-up** — specifying a timeframe (e.g., "follow-up in 7 days") and writing instructions for the AI on what to ask when the patient returns (e.g., "Ask if the antibiotics helped and if the fever subsided"). Follow-ups expire if 90 days overdue.
...
40. **Tablet kiosk mode** — clinic tablets lock into Hilt Health. Patients can't navigate away or close the app. Auto-clears session after completion.
