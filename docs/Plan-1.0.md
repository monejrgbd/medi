...
61. **Queue type per location** — each location chooses a queue mode: **Priority** (default, AI auto assigns urgency 1 to 3 based on keywords, queue sorted by priority then arrival time) or **First Come First Served** (strict arrival order, AI does not update priority, every patient waits their turn regardless of condition). Setting lives on the `locations` table as `queue_type` (`'priority'` or `'fcfs'`). In FCFS mode, all visits stay at priority 1 so existing queue ordering SQL naturally degrades to pure arrival order with no logic changes needed. The `ai-conversation` edge function checks the location's `queue_type` before calling `update_visit_priority` and skips the call entirely in FCFS mode. Owners select the queue type during onboarding (Step 1, alongside location name and specialty) with a note that the setting applies to the location being created and can be changed later in location settings. Also configurable per location in the location detail settings page. Two option cards with descriptions help owners understand the trade off.
