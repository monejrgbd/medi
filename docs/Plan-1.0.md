...
70. **Pre check in links** — receptionists generate a single use link with a patient's name pre filled via "Share Link" button on the tab bar. Link format: `/checkin/{locationId}?token={uuid}`. Patient opens the link, sees a welcome banner, confirms name (editable, fuzzy matched via pg_trgm similarity > 0.4), birthday, sex, phone. On submit, `checkin_with_token` creates the visit as `still_answering_ai` (or `waiting_doctor_claim` if `skip_ai`), skipping approval, phone verification, and discovery questions. Token is single use, expires in 48 hours, locked with `SELECT FOR UPDATE` to prevent double creation. Re opening the same link recovers the session without birthday verification (token is the auth proof). New table: `pre_checkin_tokens`. Cron cleanup deletes expired tokens after 7 days. Audit trail logs `pre_checkin_used` with name similarity score.
