...
What needs to happen before Medi processes patient health information.
...
- [ ] Set up `privacy@getmedi.ca` — the privacy policy lists this as the Privacy Officer contact
- [ ] Use a transactional email provider (Resend, Mailgun, etc.) that automatically includes unsubscribe links
- [ ] Every commercial email must include: Medi's name, mailing address, contact info, and a working unsubscribe link (CASL s.6(2))
...
- [ ] **Create a clinic-facing Data Processing Agreement template** — PHIPA s.17(3) requires a written agreement with each clinic if Medi acts as their agent. Must cover:
  - What PHI Medi can access and why
...
- [ ] **Have a PHIPA lawyer review before launch** (~$500-1000, 1-2 hours). Key questions:
  - Is Medi an "agent" or "electronic service provider" under PHIPA? This affects downstream obligations.
